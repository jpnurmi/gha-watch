...
pub(crate) fn show_main_window(app: &AppHandle, tray_rect: Option<Rect>) {
...
pub(crate) fn toggle_main_window(app: &AppHandle, tray_rect: Rect) {
    if let Some(window) = app.get_webview_window("main") {
...
#[cfg(target_os = "linux")]
pub(crate) fn linux_window_state_label(label: &str, supports_position: bool) -> &str {
...
#[cfg(target_os = "linux")]
pub(crate) fn linux_window_supports_position() -> bool {
    use gtk::gdk::prelude::*;
...
#[cfg(target_os = "linux")]
pub(crate) fn restore_linux_window_geometry(window: &tauri::WebviewWindow) {
...
#[cfg(target_os = "linux")]
pub(crate) fn save_linux_window_geometry(app: &AppHandle) {
...
#[cfg(target_os = "linux")]
pub(crate) fn configure_linux_window_frame(window: &tauri::WebviewWindow) {
    let window = window.clone();
    let handle = window.clone();
    let _ = window.run_on_main_thread(move || {
        use gtk::prelude::*;
...
#[cfg(target_os = "linux")]
pub(crate) fn configure_linux_window_controls(window: &tauri::WebviewWindow) {
    let window = window.clone();
    let handle = window.clone();
    let _ = window.run_on_main_thread(move || {
        use gtk::prelude::*;
...
#[cfg(target_os = "macos")]
pub(crate) fn configure_macos_window_frame(window: &tauri::WebviewWindow) -> tauri::Result<()> {
